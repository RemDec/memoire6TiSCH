...
Ce travail caractérise ces réseaux sans-fil restreints, leur architecture typique et les contraintes qui justifient l'élaboration d'une pile réseau adaptée. L'intégralité des différentes couches qui composent la pile 6TiSCH sont passées en revue, ainsi que la façon dont elles interagissent entre elles pour constituer une pile déployable et efficace dans un réseau sans-fil restreint. Une attention particulière est portée à la \textit{joining phase 6TiSCH}, qui dénote la façon dont un nouveau nœud rejoint un réseau 6TiSCH déjà actif. Plus particulièrement, la sécurisation de cette procédure, qui est une phase critique, est analysée en détail. Des expérimentations sont menées en simulateur pour illustrer l'impact de la sécurité sur la joining phase, et une méthode est proposée pour améliorer les performances de la joining phase.\\
...
\subsubsection{Nombre de tentatives pour le CoJP Exchange}

Afin d'expliquer en partie les résultats des mesures de temps prises avec les autres métriques, des mesures sont prises sur le Join Exchange CoJP de façon isolée. Le Join Exchange CoJP est divisé en deux parties (voir \wordlink{Section}{CoJP}) : la Join Request émise par le pledge et relayée par le Join Proxy jusqu'au JRC (nœud Root ici), et la Join Response qui est renvoyée par le Root. Cet échange constitue donc un aller-retour complet à travers tout le réseau, et est donc susceptible d'échouer et de nécessiter des retransmissions. Pour un pledge donné, on mesure d'une part le nombre de Join Requests qu'il a dû envoyer, et d'autre part le nombre de Join Responses que le nœud Root a dû renvoyer vers ce pledge pour qu'il puisse enfin rejoindre le réseau.
...
Les temps de convergence récupérés pour les 20 runs et avec/sans sécurité du join sont représentés par la \wordlink{Figure}{fig:convergence}. La \wordlink{Table}{table_convergence} reprend les statistiques calculées sur ces temps. La simulation a été paramétrée de sorte qu'elle ne se termine pas avant la convergence, et ce pour toutes les runs.
	...
	\begin{figure}[!h]
	\centering
	\includegraphics[width=0.6\linewidth]{results/secjoin/boxesTimeConvergence20.png}
	\caption{Temps de convergence d'un réseau de 10 nœuds, mesuré sur 20 runs}
...
La \wordlink{Figure}{fig:join_attempts} donne, pour chaque nœud, le nombre de tentatives pour le Join Exchange CoJP en considérant individuellement la Join Request et la Join Response (toutes runs confondues).

	\begin{figure}[!h]
	\centering
	\includegraphics[width=0.7\linewidth]{results/secjoin/boxesJoinExchange20.png}
	\caption{Nombre de tentatives effectuées pour la Join Request et la Join Response impliquées dans le join de chaque nœud}
	\label{fig:join_attempts}
	\end{figure}

\newpage

On constate que le nombre de tentatives est généralement équivalent pour les deux étapes. Ceci est logique, puisque la perte d'une Join Response renvoyée par le Root demandera au pledge d'envoyer une nouvelle Join Request vers le Root. De fait, la charge de retransmission liée au Join Exchange CoJP est répartie de façon équivalente entre le pledge et le Root. Cette charge se présente comme élevée pour certains nœuds qui sont certainement plus mal placés ou ont choisi un Join Proxy qui n'était pas le meilleur possible dans leur voisinage. Les points extrêmes donnent des valeurs allant jusqu'à 10 tentatives requises, mais généralement on observe que 2 tentatives sont suffisantes.

\subsection{Discussion globale des résultats} 

Les métriques de temps de convergence (\wordlink{Section}{results_convergence}) et temps de join par nœud (\wordlink{Section}{results_join}) n'ont pas permis de relever une différence significative entre le cas où la sécurité est activée et désactivée (ne nécessitant alors pas de Join Exchange CoJP). Cependant, les résultats de l'expérimentation présentés par la \wordlink{Section}{results_CoJP} ont montré que le Join Exchange CoJP est un échange coûteux en terme de retransmissions, induisant forcément des délais et une consommation énergétique augmentés.\\
...
À première vue, la méthode NPEB a un impact non négligable sur la rapidité avec laquelle les nœuds complètent leur processus de join. Les nœuds ont pour la grande majorité (plus de 75\%) terminé leur join après 4 minutes avec la méthode NPEB, tandis ce qu'à ce moment là, moins de 25\% ont passé l'étape de synchronisation dans un déploiement standard. En conséquence, on observe un temps de convergence moindre quand la méthode NPEB est employée. Sur ces 10 runs, la moyenne du temps de convergence est de 374.9 secondes contre 1787.3 secondes pour le déploiement standard. Bien que le nombre de runs soit trop faible pour qu'une moyenne soit représentative, les valeurs mininmum et maximum permettent de mettre en avant un net gain de performance. Effectivement, le temps de convergence maximum enregistré pour la méthode NPEB est de 629.2 secondes, ce qui représente 3 minutes de différence avec le temps minimum du déploiement standard qui est 833.1 secondes.

...
La phase de join permet le join sécurisé d'un nouveau noeud à un réseau 6TiSCH déjà établi. Le terme de ``join sécurisé" réfère à un accès authentifié au réseau, un respect des autorisations qui s'y appliquent et la distribution des paramètres nécessaires au noeud pour intégrer le réseau (clés, identifiants, etc.). La spécification \cite{ietf-6tisch-minimal-security-15} ne gère pas les mécanismes d'autorisation, mais bien les deux autres. Dans le cadre de la joining phase, les acteurs sont les suivants :
\vspace{0.1cm}
...
\vspace{0.2cm}
\item une clé partagée (\textit{PSK}, \textit{Pre-Shared Key}) : une clé secrète partagée entre le pledge et le JRC, ce dernier doit de fait stocker quelle clé est assignée à chaque pledgeID. Ces clés doivent être cryptographiquement fortes (minimum 128 bits, générée aléatoirement).
\vspace{0.2cm}
...
\subsection{Détail de la sécurité du join}

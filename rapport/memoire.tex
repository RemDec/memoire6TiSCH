...
\item {} [ blacklist ] : tableau de short identifiers dont le JRC ne souhaite pas l'acceptance des frames par le pledge (une fois qu'il deviendra joined node), permet par exemple d'isoler des noeuds corrompus ou non désirés
...
La Join Request transite dans le réseau 6TiSCH, en plusieurs sauts vraisembablement, vers le 6LBR qui lui même la relaie vers le backbone où se trouve le JRC. Il est également possible que 6LBR et JRC soient co-situés et sont en fait le même hôte. Un fois atteint le JRC, ce dernier récupère le contexte de sécurité associé au pledge. Celui-ci contient la PSK partagée avec le pledge, et le Recipient Context est calculé considérant le RID mis à la valeur prédéfinie \texttt{"JRC"}. La Recipient Key obtenue permet de déchiffrer et vérifier le payload, qui est l'objet CBOR Join\_Request.\\
...
\subsubsection{Au niveau réseau/lien}

\subsubsection{Au niveau applicatif}

...
% ------------
	...
	\item Forte densité de nœuds déployés de façon imprécise
	\item Transmissions \textit{multi-hops}
	\item Changements dans la topologie (mobilité)
...
\vspace{0.2cm}    
    ...
    \frame{
        ...
        Réseau 6TiSCH de nœuds déjà raccordés protégé au niveau L2 par les mécanismes de protection IEEE802.15.4 et \textbf{clés} distribuées par l'autorité du réseau (\textit{JRC}).\\
        \vspace{0.3cm}
        Un nœud qui veut rejoindre (\textit{pledge)} n'a pas ces clés.\\
        \vspace{0.1cm}
        Un nœud déjà raccordé fait office de \textit{Join Proxy} intermédiaire entre le pledge et l'autorité du réseau.\\
\vspace{0.1cm}        
        $\rightarrow$ émission de frame spéciales (EBs) par les nœuds déjà raccordés\\
